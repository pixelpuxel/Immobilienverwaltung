...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  const signed = verifyDocumentFileToken(params.id, "download", request.nextUrl.searchParams.get("expires"), request.nextUrl.searchParams.get("token"));
  if (signed) {
  ...
  const body = await readPrivateFile(document.storagePath);
  await auditLog({ userId: user.id, action: AuditAction.FILE_DOWNLOADED, entity: "Document", entityId: document.id, ipAddress: clientIp(request) });
  return new NextResponse(new Uint8Array(body), {
    headers: {
