...
export async function importBackupFormData(form: FormData, user: BackupImportUser): Promise<BackupImportSummary> {
  const file = form.get("file");
  if (!(file instanceof File)) throw new BackupImportError("Bitte Backup-JSON auswählen.", 400);
  ...
  if (backup.format !== "immobilienportal.backup.v1") {
    throw new BackupImportError("Backup-Format wird nicht erkannt.", 400);
  }
  ...
  if (importFiles) {
    for (const fileEntry of backup.files || []) {
      ...
      if (fileEntry.sha256 && hash !== fileEntry.sha256) {
        throw new BackupImportError(`Dateiprüfsumme stimmt nicht: ${fileEntry.path}`, 400);
      }
