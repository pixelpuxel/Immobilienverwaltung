...
async function bankingApiJson<T>(input: {
  portalInstanceId: string | null;
  path: string;
}) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new BankingApiError(response.status, String((body as { detail?: unknown; error?: unknown }).detail || (body as { error?: unknown }).error || `Banking antwortet mit HTTP ${response.status}.`), body);
    }
