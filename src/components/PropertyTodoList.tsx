...
export function PropertyTodoList({ propertyId, initialTodos }: { propertyId: string; initialTodos: TodoItem[] }) {
  ...
  return (
    <section className="rounded-lg border border-line bg-panel p-4">
      ...
      <form className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]" onSubmit={createTodo}>
...
function TodoSection({
  ...
  return (
    <div className="mt-3 grid gap-2">
      {items.map((todo) => (
        <div className="grid gap-2 rounded-md bg-white p-3 text-sm sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center" key={todo.id}>
          <input
            ...
            type="checkbox"
          />
          <span className={todo.completedAt ? "text-muted line-through" : "font-semibold"}>{todo.title}</span>
