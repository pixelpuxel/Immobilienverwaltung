...
export function AgentChatWidget() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: "assistant", content: "Hallo. Ich kann im Portal suchen, Funktionen erklären und fachliche Aktionen vorbereiten." }
  ]);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 print:hidden">
      {open ? (
        <div className="mb-3 flex h-[520px] w-[min(420px,calc(100vw-2rem))] flex-col overflow-hidden rounded-lg border border-line bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-line bg-[linear-gradient(135deg,#ecfdf5,#eef4ff)] px-4 py-3">
            ...
            <button className="button-secondary px-3 py-1 text-sm" onClick={() => setOpen(false)} type="button">Schliessen</button>
          </div>
      ...
      <button className="rounded-full bg-accent px-5 py-3 font-bold text-white shadow-xl" onClick={() => setOpen((value) => !value)} type="button">
        Agent
      </button>
    </div>
  );
